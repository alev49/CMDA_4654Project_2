...
model <- loess(y~x,data=testData, span = 1/3,degree = 1,family = "symmetric")
model$fitted
```
...
#Test example to see if the weights and coefficients match
testFit <- minPoint(10, testData, 7)
testModel <- lm(closestY~closestX, data = testFit,weights = testFit$weight)
...
myloess <- function(x, y, span = 0.5, degree = 1, show.plot = TRUE){
  ...
  for(index in 1:N{
    ...
    pointData <- minPoint(xPt = points[index], points, span*N)
    ...
    regValues <- c(regValues, pointModel$coefficients[1]+pointModel$coefficients[2]*points[index])
  }
