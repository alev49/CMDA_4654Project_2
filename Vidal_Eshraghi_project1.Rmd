...
myloess <- function(x, y, span = 0.5, degree = 1, show.plot = TRUE){
  ...
  thePlot <- ggplot()+
    ...
    labs(title=paste("LOESS Plot with", degree, "Degree Polynomial", sep = " "),
...
for(j in 1:2){
  ...
  for(i in seq(.25,.75, by=.05)){
    fit <- myloess(ozone$ozone, ozone$temperature, span=i, degree = j)
...
for(i in 1:2){
  for (j in 1:3){
    seq(.25,.75, by=.05)
    fit <- myloess(ozone$ozone, ozone$temperature, span=i, degree = i)
  }
}
bestfitD1 <- myloess(ozone$ozone, ozone$temperature, span=.25, degree = 1)
```
