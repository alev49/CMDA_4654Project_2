...
# Specific function that finds the distances between the 
# k neighbors from the specifc vector (newPt) data point
# from the data frame of points
# Will return a data frame of the k distances from newPt
findNeighborDist <- function(newPt, points, k){
...
mykNN <- function(train, test, y_train, y_test, k = 3, weighted = TRUE){
  ...
  resultClass <- list(Predicted_Responses=yhat, accuracy=acc, Error_Rate=1-acc, Confusion_Matrix=confMat, Value_of_k=k)
  
  resultReg <- list(Predicted_Responses=yhat, Residual_Vector=resid, SSE=sse, Value_of_k=k)
