...
myloess <- function(x, y, span = 0.5, degree = 1, show.plot = TRUE){
  ...
  thePlot <- ggplot()+
    ...
    geom_line(data = fittedData, aes(x=x, y=y, colour="My LOESS"))+
    geom_line(data = truData,aes(x=x,y=y, colour="R LOESS"))+
    labs(title=paste(" LOESS Plot with", degree, "Degree Polynomial", "\n", "with Span:", span, sep = " "),
        x ="x variable", y = "y variable", colour="Legend")
...
findNeighbors(Auto_new[41,-8], Auto_new[,-8], 7)
...
mykNN <- function(train, test, y_train, y_test, k = 3, weighted = TRUE){
  ...
  for(point in 1:nrow(test)){
    #Holds the classified predictions
    ...
    #Goes through using the weighted method to figure out classification for each point
    if(weighted & !regression){
      ...
      neighbors$c <- neighbors$weight/sum(neighbors$weight)
      yhat <- sum(neighbors$c*point)
      classified <- c(classified, yhat)
    }
    else{
      #Finds out largest occurrence of specific classification and stores it
      classified <- names(table(neighborClass))[which.max(table(neighborClass))]
    }
  ...
  if(!regression){
    # Creates confusion Matrix that shows for the predicted classifications versus our 
    # correct test set
...
model <-mknn(train =Auto_new[dt, -8] , test = Auto_new[-dt, -8], k = 3, cl =Auto_new[dt, 8])

mykNN(train=Auto_new[dt, -8], test = Auto_new[-dt, -8], y_test = Auto_new[-dt, 8], y_train = Auto_new[dt, 8], k = 5)

b <- c("b","b","b","a","a","b")
s <- c("b","b","a","a","b","b")
matty <- confusionMatrix(data = as.factor(b), reference = as.factor(s))
table(b,s)
names(table(b))[which.max(table(b))]
max(table(b),)
max(table(b,deparse.level = 0))
(length((unique(b) == b)[(unique(b) == b) == TRUE]))
```
